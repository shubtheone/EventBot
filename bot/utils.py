...
def create_event_embed(event: Dict) -> discord.Embed:
    ...
    embed = discord.Embed(
        ...
        color=0xE74C3C if source == "Unstop" else 0x2ECC71,
    )
    ...
    if source == "Unstop":
        fee = event.get("fee", "N/A")
        embed.add_field(name="Registration", value=fee, inline=True)

        prize = event.get("prize", "Not specified")
        embed.add_field(name="Prizes", value=prize, inline=False)

        reg_count = event.get("registrations", 0)
        embed.add_field(name="Registrations", value=str(reg_count), inline=True)

        embed.add_field(name="Link", value=url or "N/A", inline=False)
    else:
        # CTFtime fields
