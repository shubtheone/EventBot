...
class EventView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Create Channel", style=discord.ButtonStyle.green, emoji="➕"
    )
    async def create_channel(
        ...
        if self.event_cog.get_channel_id(guild_id, event_url):
            await interaction.followup.send(
                "A channel for this event already exists!", ephemeral=True
            )
            return
        ...
        try:
            ...
            await channel.send(
                f"Welcome to the war room for **{title}**! @here", embed=embed
            )

            # Register active event
            self.event_cog.add_active_event(
                guild_id, event_url, channel.id, self.event_data
            )

            await interaction.followup.send(
                f"✅ Created channel {channel.mention}!", ephemeral=True
            )

        except Exception as e:
            await interaction.followup.send(
                f"Failed to create channel: {str(e)}", ephemeral=True
            )

    @discord.ui.button(
        label="Delete Channel", style=discord.ButtonStyle.red, emoji="➖"
    )
    async def delete_channel(
        ...
        if not channel_id:
            await interaction.followup.send(
                "No tracked channel found for this event.", ephemeral=True
            )
            return
        ...
        if channel:
            try:
                ...
                await interaction.followup.send(
                    f"Failed to delete channel: {e}", ephemeral=True
                )
        else:
            # Channel already gone, just clean up DB
            self.event_cog.remove_active_event(guild_id, event_url)
            await interaction.followup.send(
                "Channel record cleaned up (channel was missing).", ephemeral=True
            )
...
class Events(commands.Cog):
    def __init__(self, bot):
        ...
        self.active_events = self._load_json(
            ACTIVE_EVENTS_FILE, {}
        )  # Structure: {guild_id: {event_url: {data}}}
    ...
    def get_channel_id(self, guild_id, event_url):
        return (
            self.active_events.get(str(guild_id), {})
            .get(event_url, {})
            .get("channel_id")
        )
    ...
    @commands.hybrid_command(
        name="hackathons",
        description="List upcoming CTF/Cybersecurity events from Unstop",
    )
    async def hackathons(self, ctx, limit: int = 5):
        await ctx.defer()
        await self._send_events(ctx, limit, type_filter="Unstop")
    ...
    @commands.hybrid_command(
        name="watch", description="Set this channel to receive new CTF notifications"
    )
    ...
    async def watch(self, ctx):
        ...
        await ctx.send(
            f"✅ checks enabled! I will post new events to {ctx.channel.mention}."
        )
                ...
                for guild_id, channel_id in self.subscriptions.items():
                    ...
                    if channel:
                        for event in new_events:
                            ...
                            view = EventView(self.bot, event, self)
                            await channel.send(
                                "🚨 **New Event Detected!**", embed=embed, view=view
                            )
            else:
